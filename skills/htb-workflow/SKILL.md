...
[9] 收尾: 双 flag（🔴 搜全盘非默认位置）+ 恢复被改密码 + 沉淀进记忆（memory MCP 或工作区笔记）
```text
...
[2] 卡住（同方向 ≥2 次失败）→ 强制三查（缺一不可）:
    memory search "<错误码/技术词>" + history search "<错误码>" + 记忆图谱 search（有 memory MCP 的会话）
    → 输出困境分类标签（格式见 blocking-points-detail 卡）→ 才允许继续
[3] 读文档/WP/网页 → tavily_extract / web_search 优先（禁止 curl+python 硬解析 HTML）
...
收尾强制三步: 记忆沉淀 / 技能反哺 / 变更复盘 → box-startup 阶段5
```text
...
[1] 工具报协议错误（BADOPTION/ETYPE_NOSUPP/STATUS_NOT_SUPPORTED）
    → 先 skill 工具按名加载匹配的卡 + 搜索（HackTricks/NetExec Wiki），禁止改源码试错（见 blocking-points-detail 卡）
...
[7] 🔴 卡住三查必含 history: memory + history + 记忆图谱（若有）（PingPong 经验躺在 history 里没被查）
