...
whenToUse: '遇到具体技术环节时按名查直达 URL，直接用 mcp__tavily__tavily_extract（Windows 教练会话）或 web_search，不绕搜索引擎。'
...
> 🔴 遇到技术环节 → 先查本索引 → 用 mcp__tavily__tavily_extract（Windows 教练会话）或 web_search 直接取对应页面（内容完整可读），不需要搜索引擎绕路
...
```
技术环节动手前:
  [1] memory MCP search（仅 Windows 教练会话）查 "<技术关键词>"        ← 查自己的资产
  [2] 查本索引 → mcp__tavily__tavily_extract（Windows 教练会话）或 web_search 取对应 HackTricks 页面  ← 直达官方 wiki
...
**How to apply:** 进入会话目录（模型按需加载）；遇到上表技术先查索引再动手；新技术发现后补录本索引（URL 用 `curl -sI` 验证 200）。
